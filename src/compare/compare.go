...
func Compare(obj1, obj2 map[string]interface{}) []Diff {
	...
	for _, key := range commonKeys {
		...
		case !ok && yes:
			...
			diffs = append(diffs, Diff{DifTest: map[string]any{key: value2}, Message: " # Добавлена", Type: "+ "})
		case ok && !yes:
			//strData1 := fmt.Sprintf(" - %v: %v", key, value1)
			diffs = append(diffs, Diff{DifTest: map[string]any{key: value1}, Message: " # Удалена", Type: "- "})
		...
		default:
			...
			if isSimple1 && isSimple2 && value1 != value2 {
				//strData1 := fmt.Sprintf(" - %v: %v", key, value1)
				diffs = append(diffs, Diff{DifTest: map[string]any{key: value1}, Message: " # Старое значение", Type: "- "})
				//strData2 := fmt.Sprintf(" + %v: %v", key, value2)
				diffs = append(diffs, Diff{DifTest: map[string]any{key: value2}, Message: " # Новое значение", Type: "+ "})
				// Оба простых типа и значения равны
			} else if isSimple1 && isSimple2 && value1 == value2 {
				//strData1 := fmt.Sprintf("   %v: %v", key, value1)
				diffs = append(diffs, Diff{DifTest: map[string]any{key: value1}, Message: "", Type: "  "})
